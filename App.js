...
client.on("roleDelete", async (role) => {
  ...
  return new Promise(async function (resolve) {
    ...
    ChannelModel.updateMany({ "Permissions.id": role.id }, { $set: { "Permissions.$.id": nrole.id } }).exec(async (e,res) => {
      if(e) return;
    
      let _bot = giveBot(1)[0];
      let channelData = await ChannelModel.find({ "Permissions.id": nrole.id });
      if(!channelData || nrole.deleted) return;
    
      for (let index = 0; index < channelData.length; index++) {
        const element = channelData[index];
        if(nrole.deleted) break;
        let _channel = _bot.guilds.cache.get(SERVER_ID).channels.cache.get(element.Id);
        if(!_channel) continue;
        _channel.edit({ type: element.Type, permissionOverwrites: element.Permissions });
      };
    });
